  ...
  return(
    ...
    <div>
     ...
     <div className="absolute left-6 lg:left-12 md:hidden block" onClick={navigateBack}><FaArrowLeft className="text-xl"/></div>
      <div> <input type="text" ref={inputRef} value={input} onChange={handleInput} placeholder="Search products, brands and categories" className="input-width w-60 sm:w-96 md:pl-12 ml-12 md:ml-0 text-base pr-4 md:border-r md:border  md:pr-0  placeholder:text-base  border-r-2 outline-none lg:pt-2 lg:pb-2 lg:w-150 xl:w-260 md:w-98 md:pt-1.5 md:pb-1.5   border-gray-300" /> </div> 
     ...
     {input.length > 0 ? (   <div className="absolute screen-size sm:right-44 right-20"><FaTimes className="text-gray-400 text-lg"/></div>):null}
